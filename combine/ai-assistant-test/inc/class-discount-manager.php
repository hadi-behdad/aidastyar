...
class AI_Assistant_Discount_Manager {
    ...
    private function maybe_create_table() {
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '{$this->table_name}'") != $this->table_name) {
            ...
            $sql = "CREATE TABLE {$this->table_name} (
                ...
                scope varchar(20) NOT NULL, -- global, service, coupon, user_based
                service_ids text DEFAULT NULL, -- برای scope=service یا coupon
                coupon_code varchar(100) DEFAULT NULL, -- برای scope=coupon
                start_date datetime DEFAULT NULL,
                end_date datetime DEFAULT NULL,
                usage_limit int(11) DEFAULT 0, -- 0 = unlimited
                usage_count int(11) DEFAULT 0,
                user_restriction varchar(20) DEFAULT NULL, -- first_time, specific_users
                user_ids text DEFAULT NULL, -- برای کاربران خاص
    ...
    private function render_edit_form($discount_id = 0) {
        ...
        echo '<td><input type="number" id="discount_value" name="discount_value" value="' . 
             ($discount ? esc_attr($discount->discount_value) : '') . '" step="0.01" min="0" required class="small-text">';
        ...
        echo '<td><select id="service_ids" name="service_ids[]" multiple class="regular-text">';
        ...
        echo '<select id="user_ids" name="user_ids[]" multiple class="regular-text">';
        ...
        echo '<td><input type="number" id="min_order_amount" name="min_order_amount" value="' . 
             ($discount ? esc_attr($discount->min_order_amount) : '0') . '" step="0.01" min="0" class="small-text"> تومان</td></tr>';
    ...
    private function render_discounts_list() {
        ...
        echo '<a href="' . admin_url('admin.php?page=ai-assistant-discounts&action=add') . '" class="button button-primary">افزودن تخفیف جدید</a>';
        echo '<hr>';
    ...
    private function sanitize_discount_data($data) {
        ...
        switch ($sanitized['scope']) {
            case 'service':
                $sanitized['service_ids'] = !empty($data['service_ids']) ? implode(',', array_map('intval', $data['service_ids'])) : '';
            ...
            case 'user_based':
                ...
                if ($sanitized['user_restriction'] == 'specific_users' && !empty($data['user_ids'])) {
                    $sanitized['user_ids'] = implode(',', array_map('intval', $data['user_ids']));
                } else {
                    $sanitized['user_ids'] = null;
                }
                $sanitized['service_ids'] = !empty($data['service_ids']) ? implode(',', array_map('intval', $data['service_ids'])) : '';
     ...
     */
    private function get_applicable_discounts($service_id, $user_id = 0, $coupon_code = '') {
        global $wpdb;
